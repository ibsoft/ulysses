...
from ..config import (
    ...
    KOKORO_SAMPLE_RATE,
    KOKORO_TRAILING_SILENCE_SAMPLES,
...
def get_voices(path: Path | None = None) -> list[str]:
    ...
    """
    voices = np.load(path or VOICES_PATH)
...
class SpeechSynthesizer:
    ...
    MODEL_PATH: Path = TTS_KOKORO_MODEL_PATH
    DEFAULT_VOICE: str = DEFAULT_KOKORO_VOICE
    MAX_PHONEME_LENGTH: int = KOKORO_MAX_PHONEME_LENGTH
    ...
    def _get_vocab() -> dict[str, int]:
        ...
        _letters_ipa = (
            "ɑɐɒæɓʙβɔɕçɗɖðʤəɘɚɛɜɝɞɟʄɡɠɢʛɦɧħɥʜɨɪʝɭɬɫɮʟ"
            "ɱɯɰŋɳɲɴøɵɸθœɶʘɹɺɾɻʀʁɽʂʃʈʧʉʊʋⱱʌɣɤʍχʎʏʑʐʒ"
    ...
    def _synthesize_ids_to_audio(self, ids: list[int]) -> NDArray[np.float32]:
        ...
        return np.array(
            audio[:-KOKORO_TRAILING_SILENCE_SAMPLES],
            dtype=np.float32,
