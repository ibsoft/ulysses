...
from ..config import (
    ASR_TDT_CONFIG_PATH,
    ASR_TDT_DECODER_MODEL_PATH,
    ASR_TDT_ENCODER_MODEL_PATH,
    ASR_TDT_JOINER_MODEL_PATH,
    configure_onnxruntime_logging,
    get_onnx_providers,
)
from .audio_file import load_transcription_audio
...
class _OnnxTDTModel:
    ...
    DEFAULT_ENCODER_MODEL_PATH = ASR_TDT_ENCODER_MODEL_PATH
    DEFAULT_DECODER_MODEL_PATH = ASR_TDT_DECODER_MODEL_PATH
    DEFAULT_JOINER_MODEL_PATH = ASR_TDT_JOINER_MODEL_PATH
...
class AudioTranscriber:
    ...
    DEFAULT_CONFIG_PATH = ASR_TDT_CONFIG_PATH
    DEFAULT_ENCODER_MODEL_PATH = _OnnxTDTModel.DEFAULT_ENCODER_MODEL_PATH
    DEFAULT_DECODER_MODEL_PATH = _OnnxTDTModel.DEFAULT_DECODER_MODEL_PATH
    DEFAULT_JOINER_MODEL_PATH = _OnnxTDTModel.DEFAULT_JOINER_MODEL_PATH
    ...
    def __init__(
        ...
        providers = get_onnx_providers(prefer_cuda=True)

        # Initialize the internal ONNX model handler
        self.model = _OnnxTDTModel(
            providers,
            encoder_model_path=encoder_model_path,
            decoder_model_path=decoder_model_path,
            joiner_model_path=joiner_model_path,
        )
    ...
    def transcribe_file(self, audio_path: Path) -> str:
        ...
        audio = load_transcription_audio(audio_path, self.melspectrogram.sample_rate)
        logger.info(
            f"Audio ready: duration={len(audio) / self.melspectrogram.sample_rate:.2f}s, "
            f"sample_rate={self.melspectrogram.sample_rate}Hz"
        )
