...
from ..config import ASR_VAD_MODEL_PATH, INPUT_SAMPLE_RATE, configure_onnxruntime_logging, get_onnx_providers
...
class VAD:
    VAD_MODEL: Path = ASR_VAD_MODEL_PATH
    ...
    def __init__(self, model_path: Path = VAD_MODEL) -> None:
        ...
        self.avaliable_sample_rates = [self.SAMPLE_RATE]
    ...
    def __call__(self, audio_sample: NDArray[np.float32], sample_rate: int = SAMPLE_RATE) -> NDArray[np.float32]:
        ...
        if sample_rate != self.SAMPLE_RATE:
            raise ValueError(f"silero_vad_16k_op15.onnx only supports {self.SAMPLE_RATE} Hz audio.")
        num_samples = 512
        ...
        context_size = 64 if sample_rate == self.SAMPLE_RATE else 32
