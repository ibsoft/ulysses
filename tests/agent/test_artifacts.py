...
from sirina_agent.core.assessment import assessment_tool_options, select_assessment_tool_options
...
def test_assessment_tool_selection_accepts_numbers_names_and_all():
    ...
    selected, error = select_assessment_tool_options("1, nmap, tls", options)
    assert error is None
    assert [item.id for item in selected] == ["dns", "service-scan", "tls"]

    selected, error = select_assessment_tool_options("all", options)
    assert error is None
    assert selected == options

    selected, error = select_assessment_tool_options("maybe later", options)
    assert selected == []
